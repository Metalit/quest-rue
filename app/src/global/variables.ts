...
export async function updateReferenceVariables() {
  const { addresses } = await sendPacketResult<GetSafePtrAddressesResult>({
    getSafePtrAddresses: {},
  })[0];
...
export function firstFree(beginning = "unnamed_variable", ignore?: string) {
  ...
  Object.keys(variables).forEach((name) => {
    if (name !== ignore && name.startsWith(beginning)) {
      const end = name.substring(beginning.length).trim();
  ...
  if (num == 0) return beginning;
  return `${beginning}_${num}`;
}
...
export function validVariableName(name: string) {
  const unicodeReplaced = name.replace(
